...
logger = logging.getLogger("rezerwuj.email")
...
def send_booking_confirmation_email(
    ...
    """Wysyła potwierdzenie rezerwacji e-mailem."""
    biz_name = company_name or provider_name
    subject = f"Potwierdzenie rezerwacji u {biz_name}"
...
<h2 style="color:#0d6efd;">✅ Rezerwacja potwierdzona</h2>
<p>Dzień dobry, <strong>{client_name}</strong>!</p>
<p>Twoja rezerwacja u <strong>{biz_name}</strong> została przyjęta.</p>
...
<tr><td style="padding:6px 12px;font-weight:bold;border:1px solid #dee2e6;">Usługodawca</td><td style="padding:6px 12px;border:1px solid #dee2e6;">{biz_name}</td></tr>
</table>
<p style="color:#6c757d;font-size:14px;">Dziękujemy! Do zobaczenia.<br>— {biz_name}</p></body></html>"""
...
def send_booking_reminder_email(
    ...
    biz_name = company_name or provider_name
    subject = f"Przypomnienie: rezerwacja u {biz_name} jutro"
    html = f"""<!DOCTYPE html><html><head><meta charset="utf-8"></head><body style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:20px;">
<h2 style="color:#0d6efd;">⏰ Przypomnienie o wizycie</h2>
<p>Dzień dobry, <strong>{client_name}</strong>!</p>
<p>Przypominamy o Twojej wizycie u <strong>{biz_name}</strong>:</p>
<table style="border-collapse:collapse;margin:16px 0;">
<tr><td style="padding:6px 12px;font-weight:bold;border:1px solid #dee2e6;">Data</td><td style="padding:6px 12px;border:1px solid #dee2e6;">{date}</td></tr>
<tr><td style="padding:6px 12px;font-weight:bold;border:1px solid #dee2e6;">Godzina</td><td style="padding:6px 12px;border:1px solid #dee2e6;">{time}</td></tr>
</table>
<p style="color:#6c757d;font-size:14px;">Do zobaczenia!<br>— {biz_name}</p></body></html>"""
    return send_email(to, subject, html)
...
<p>Otrzymaliśmy prośbę o zresetowanie hasła dla Twojego konta w systemie <strong>Rezerwuj</strong>.</p>
...
<p style="color:#6c757d;font-size:12px;">Rezerwuj — {SITE_URL}</p></body></html>"""
    return send_email(to, subject, html)
